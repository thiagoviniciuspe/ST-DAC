...
os.environ['THEANO_FLAGS']='device=cuda0,gpuarray.preallocate=0.1,mode=FAST_RUN,floatX=float32,optimizer=fast_compile,dnn.enabled=True,exception_verbosity=high'
...
index_begin = np.arange(X_train.shape[0])
np.random.shuffle(index_begin)
X_train = X_train[index_begin]
y_true = y_true[index_begin]

tempmap = np.copy(index_begin)
#parameters
batch_size = 32
epoch = 10
nb_epoch = 1
...
x = BatchNormalization(mode=2,axis = 1)(x)
x = Activation('relu')(x)
x = SpatialTransformer(localization_net=modelLoc2, downsample_factor=1, name='spatial12')(x)
x = MaxPooling2D(pool_size = (2,2))(x)
x = BatchNormalization(mode=2,axis = 1)(x)
...
for e in range(epoch):
	...
	if e == 9:
		index_begin = np.sort(index_begin)
		X_train = X_train[index_begin]
		y_true = y_true[index_begin]
		Output = cluster_l1.predict(X_train)
		y_pred = np.argmax(Output,axis = 1)
		for i in range(0,10):
			idxarr = np.nonzero(y_pred == i)
			predsel = predictions[idxarr]
			x_train_zeros = X_train[y_pred == i]
			for j in range(0,x_train_zeros.shape[0]):
				outputl = '/home/Workspace/STDAC/mnistclusters/'+str(i)+'/mnist'+str(idxarr[0][j])+str(y_true[idxarr[0][j]])+'.png'
				scipy.misc.imsave(outputl, x_train_zeros[j].reshape(28,28))
				outputl = '/home/Workspace/STDAC/mnistclusters/'+str(i)+'/stmnist'+str(idxarr[0][j])+str(y_true[idxarr[0][j]])+'.png'
				scipy.misc.imsave(outputl,predsel[j].reshape(28,28))
