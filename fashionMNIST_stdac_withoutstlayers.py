...
#
#Experiment: ST-DAC without ST-Layers
...
x = Convolution2D(64,3,3,init = 'he_normal',border_mode='same', input_shape=(1,28,28))(inp_)
...
for e in range(epoch):
	...
	plt.show()
	fname = ""
	fname = '/small_fashion_mnist_spatial_front'+str(e)+'.png'
	plt.savefig(location.replace('.h5',fname))
	plt.close()
	if ((e%2)==0) :
		upper = upper - etau
	else:
	...
	if e == 9:
		...
		for i in range(0,10):
			...
			for j in range(0,x_train_zeros.shape[0]):
				outputl = '/home/Workspace/STDAC/fahionmnistclusterstdac/'+str(i)+'/fashionmnist'+str(y_true[idxarr[0][j]])+str(idxarr[0][j])+'.png'
