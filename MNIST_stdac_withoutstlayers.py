...
os.environ['THEANO_FLAGS']='device=cuda1,gpuarray.preallocate=0.5,mode=FAST_RUN,floatX=float32,optimizer=fast_compile,dnn.enabled=True,exception_verbosity=high'
...
for e in range(epoch):
	...
	for i in range(1,n):
		...
		ax.get_yaxis().set_visible(False)
	plt.show()
	plt.savefig('mnist_spatial_'+str(e)+'.png')
	plt.close()
	if ((e%2)==0) :
		upper = upper - etau
	else:
